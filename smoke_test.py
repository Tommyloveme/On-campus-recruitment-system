...
# 5a-master. 主数据表配置（上传/刷新由根目录真实双 Excel 与单元测试覆盖）
...
s, c_term = call("POST", "/api/candidates", {
    "stage": "registration",
    "data": sample_reg_data("终止恢复测试"),
})
check("终止恢复测试候选人创建成功", s == 200 and c_term.get("id"))
cid_t = c_term["id"]
stage_before_term = c_term.get("current_stage") or "registration"
s, r = call("POST", f"/api/candidates/{cid_t}/terminate", {"action": "terminate"})
check("流程终止成功", s == 200 and r.get("terminated"))
s, ct = call("GET", "/api/candidates?q=" + quote("终止恢复测试"))
check("终止后流程状态为流程终止",
