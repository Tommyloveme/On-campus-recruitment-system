...
P("3.3 Excel 导入", S_H2)
LI("先点「下载导入模板」获得与当前字段配置完全匹配的表头模板（首列为「三层部门」，其次为「候选人」；二层部门由导入时分组指定，不在模板中）。")
LI("点「Excel 导入」选择目标分组并上传 .xlsx 文件；表头按字段配置自动匹配（含旧表头别名，如「入职三层」仍识别为三层部门），模板外的列会被忽略，缺失的列不导入。")
LI("已存在的候选人（电话或姓名相同）自动更新，其余新增；导入结果（新增/更新/跳过数量）写入操作日志。")
P("3.4 导出", S_H2)
LI("勾选行首复选框（表头复选框 = 全选当前筛选结果）。")
LI("「导出选中Excel」：将选中候选人按当前字段配置导出为 .xlsx 文件（首列为二层部门/权限分组名，其余列顺序同导入模板）。")
...
LI("维度（横轴）：任意可见列或二层部门；选择日期列时可再选日期粒度——按年月日 / 按年月 / 按月份（跨年聚合）。")
LI("系列（图例）：可选字段与维度相同（含二层部门、文本/日期/状态列），用于交叉分析，如「三层部门 × 签约状态」或「毕业时间 × 拟录取工作地」。")
